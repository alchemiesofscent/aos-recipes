...
async function init() {
  try {
    if (window.AETIUS_BOOK1_CH99_136_DATA) {
      chapterData = window.AETIUS_BOOK1_CH99_136_DATA;
    } else {
  ...
  document.getElementById("chapter-name").textContent = `Chapters 100-136 from the Olivieri edition · proemium title: ${chapterData.proemium[0].chapter_name}`;
  renderProemium();
  bindNavigation();
  renderEntry();
}
