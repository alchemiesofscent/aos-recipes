...
async function init() {
  try {
    if (window.DIOSCORIDES_BOOK1_DATA) {
      chapterData = window.DIOSCORIDES_BOOK1_DATA;
    } else {
      const response = await fetch("data/dioscorides_book1_perfumes_resins.json");
      if (!response.ok) {
  ...
  document.getElementById("chapter-name").textContent =
    `Perfume and resin units from the raw Wellmann witness. Proemium slice: chapter 42 section 2 (raw lines 2306-2342); the carousel begins with chapter 25 κῦφι and continues through chapters 30-63 plus chapter 71 resin techniques.`;
  renderSourceNotes();
  renderProemium();
  bindNavigation();
  renderEntry();
    ...
    .map(
      (section) => `
        <section class="proemium-block">
          <div class="proemium-meta">Chapter ${escapeHtml(section.chapter)} · Section ${escapeHtml(section.section)} · Wellmann ${escapeHtml(section.wellmann.citation.start)}-${escapeHtml(section.wellmann.citation.end)}</div>
